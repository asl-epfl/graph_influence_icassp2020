...
ax.set_ylim(-8,8)
plt.axis('off')
nx.draw_networkx_nodes(G_full, pos=pos, node_color= 'C0', vmin=0, vmax= 2, nodelist = range(NS[0]),node_size=600, edgecolors='k', linewidths=.5)
nx.draw_networkx_nodes(G_full, pos=pos, node_color = 'C1', vmin=0, vmax= 2, nodelist = range(NS[0], NS[0]+NS[1]), node_size=600, edgecolors='k', linewidths=.5)
nx.draw_networkx_nodes(G_full, pos=pos, node_color = 'C2', vmin=0, vmax= 2, nodelist = range(NS[0]+NS[1],sum(NS)), node_size=600, edgecolors='k', linewidths=.5)
nx.draw_networkx_nodes(G_full, pos=pos, node_color = '#E2C458', vmin=0, vmax= 2, nodelist = range(sum(NS),N_T), node_size=600, edgecolors='k', linewidths=.5)

circle1 = mpl.patches.Circle((-.2, 3.5), 3.5, fc='None', ec='k', linewidth=0.8)
circle2 = mpl.patches.Circle((6.5, 3.9), 3, fc='None', ec='k', linewidth=0.8)
circle3 = mpl.patches.Circle((4.3, -4.5), 3.2, fc='None', ec='k', linewidth=0.8)
circle4 = mpl.patches.Circle((12.4, 3.2), 2.8, fc='None', ec='k', linewidth=0.8)
ax.add_artist(circle1)
ax.add_artist(circle2)
ax.add_artist(circle3)
ax.add_artist(circle4)
nx.draw_networkx_labels(G_full,pos,{i: i+1 for i in range(N_T)},font_size=16, font_color='black', alpha = 1)
nx.draw_networkx_edges(G_full, pos = pos, node_size=500, alpha=1, arrowsize=6, width=0.5);
plt.savefig(FIG_PATH + 'fig1_panel1.pdf', bbox_inches='tight')
#%%
np.random.seed(5)
...
for i in range(4):
    ...
    if i==0:
        ...
        plt.annotate('%0.2f' % (A_lim[1*NS[0]:1*NS[0]+NS[0],i+sum(NS)].sum()-.01), xy=(1.01,A_lim[1*NS[0]:1*NS[0]+NS[0],i+sum(NS)].sum()-0.04), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
        ...
        plt.annotate('%0.2f' % (A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum()+.01), xy=(1.01,A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum()+.1), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
        ...
        plt.annotate('%0.2f' % (A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum()), xy=(1.01,A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum()), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
        plt.annotate('%0.2f' % (A_lim[1*NS[0]:1*NS[0]+NS[0],i+sum(NS)].sum()), xy=(1.01,A_lim[1*NS[0]:1*NS[0]+NS[0],i+sum(NS)].sum()-.04), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
        plt.annotate('%0.2f' % A_lim[2*NS[0]:2*NS[0]+NS[0],i+sum(NS)].sum(), xy=(1.01,A_lim[2*NS[0]:2*NS[0]+NS[0],i+sum(NS)].sum()-0.05), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
    elif i==3:
        plt.annotate('%0.2f' % A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum(), xy=(1.01,A_lim[0*NS[0]:0*NS[0]+NS[0],i+sum(NS)].sum()), xycoords=('axes fraction', 'data'), color = 'r', fontsize=13)
...
plt.savefig(FIG_PATH+'fig1_panel3.pdf', bbox_inches='tight')
#%%
plt.figure(figsize=(5,4.5))
for i in range(4):
    plt.subplot(2,2,i+1)
...
plt.savefig(FIG_PATH+'fig1_panel2.pdf', bbox_inches='tight')
